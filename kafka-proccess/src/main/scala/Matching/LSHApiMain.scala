...
object LSHApiMain extends App {
  ...
  implicit val oppFormat: RootJsonFormat[OpportunityRequest] =
    jsonFormat5(OpportunityRequest)

  implicit val studentFormat: RootJsonFormat[StudentResponse] =
    jsonFormat8(StudentResponse)
  ...
  private def withCors(route: akka.http.scaladsl.server.Route) =
  ...
  implicit val rejectionHandler: RejectionHandler =
    RejectionHandler.newBuilder()
      .handleNotFound {
        complete(jsonError(StatusCodes.NotFound, "Route not found"))
      }
      .result()
  ...
  val route =
    withCors {
      path("match_students") {
          ...
          entity(as[String]) { body =>
            Try(body.parseJson.convertTo[OpportunityRequest]) match {
              ...
              case Success(req) =>
                ...
                if (skills.isEmpty && courses.isEmpty && major.isEmpty) {
                  complete(jsonError(
                    StatusCodes.BadRequest,
                    "Request must include skills or courses or major"
                  ))
                } else {
        ...
        pathSingleSlash {
          get {
            complete("LSH API is running. POST /match_students")
          }
        }
    }
